...
// ---------------------------------------------------------------------------
// Direct (non-Inngest) batch runner — same logic as batchRunFunction but uses
// plain await instead of step.run(). Intended for dev via Next.js after().
// ---------------------------------------------------------------------------
export async function runBatchDirect(batchId: string): Promise<void> {
  // -----------------------------------------------------------------------
  // Step 1: Detect mode and load data
  // -----------------------------------------------------------------------
  const { mode, batch, property, generations, legacyPhotos } = await (async () => {
    ...
    await Promise.all(
      Array.from(roomGroups.entries()).map(async ([roomKey, roomGens]) => {
        ...
        for (const gen of ordered) {
          ...
          // even while staging is in progress. Idempotent — safe on retries.
