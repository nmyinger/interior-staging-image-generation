...
export default async function SessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (access.needsPassword && access.passwordHash) {
    ...
    if (!verifyPasswordCookie(cookieValue, id, access.passwordHash)) {
      redirect(`/session/${id}/password`);
    }
  }

  const rows = await sql`SELECT name, owner_user_id FROM sessions WHERE id = ${id}`;
  ...
  return (
    <main className="flex flex-col h-screen bg-stone-50">
      <SessionHeader sessionId={id} initialName={name} ownerUserId={owner_user_id} readOnly={readOnly} />
