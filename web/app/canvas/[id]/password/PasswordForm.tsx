...
export function PasswordForm({ canvasId }: { canvasId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/sessions/${canvasId}/verify-password`, {
        method: "POST",
      ...
      <div className="px-6 py-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-500">Enter the password to view this canvas</p>
          </div>
        </div>
